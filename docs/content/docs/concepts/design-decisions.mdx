...
Signals are a proven reactive pattern. Angular, Solid, Preact, Vue, Svelte, and Qwik each implement them differently, but the core idea is the same: a value that notifies its consumers when it changes. React is the exception - it has no built-in signal primitive. This library brings signals to React with a small, framework-agnostic core and a thin React bridge.
...
Both `untracked(fn){:ts}` for arbitrary code and `untracked(signal){:ts}` for a single signal are supported - inspired by Angular. Most frameworks offer some form of untracked reads, except Vue which has no direct equivalent. Preact uses `.peek(){:ts}` on individual signals instead of a global function - though the name does not convey "untracked", which can surprise developers who see it return a value without realizing it skips dependency tracking.
