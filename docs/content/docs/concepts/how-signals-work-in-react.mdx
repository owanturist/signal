...
React's built-in state primitives (`useState{:ts}`, `useReducer{:ts}`) use a coarse-grained update model: when state changes, the entire component and its children re-render. You can optimize this with `memo{:ts}`, `useMemo{:ts}`, and `useCallback{:ts}`, but it requires manual work.
...
3. Each signal records the Monitor as a consumer
...
The `useComputed{:ts}` hook provides a `Monitor{:ts}` to your compute function:
...
The `SearchPage` component creates the signal and passes it down. Both children read independently - typing in the input updates the results without re-rendering `SearchPage`.
