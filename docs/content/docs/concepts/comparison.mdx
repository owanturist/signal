...
const count = Signal(0) // [!code focus]
...
function Counter() {
  ...
  const [count] = useState(() => Signal(0)) // [!code focus]
  return <CounterDisplay count={count} />
}
```
...
const counterSlice = createSlice({ // [!code focus:7]
  name: "counter",
...
const store = configureStore({ // [!code focus:3]
...
function App() {
  return (
    <Provider store={store}> // [!code focus:4]
      <Counter />
    </Provider>
  )
}
...
function Counter() {
  const count = useSelector((state) => state.counter.count) // [!code focus]
...
const useCounter = create<{ // [!code focus:7]
  count: number;
  increment: () => void
}>((set) => ({
  count: 0,
...
function Counter() {
  const count = useCounter((state) => state.count) // [!code focus]
  return <span>{count}</span>
}
```
...
const countAtom = atom(0) // [!code focus]

// Values live in an implicit global store — no Provider needed
function Counter() {
  const count = useAtomValue(countAtom) // [!code focus]
  return <span>{count}</span>
}
```
...
function Counter() {
  const actorRef = useActorRef(counterMachine) // [!code focus]
  const count = useSelector(actorRef, (snapshot) => snapshot.context.count) // [!code focus]
  return <span>{count}</span>
}

// Shared scope — createActorContext provides a Provider
const CounterContext = createActorContext(counterMachine) // [!code focus]
```
...
function CounterBadge({ count }: { count: ReadableSignal<number> }) {
  const value = useComputed(count) // [!code focus]
...
function CounterBadge() {
  const count = useSelector((state: RootState) => state.counter.count) // [!code focus]
  return <span>{count}</span>
}
```
...
function CounterBadge() {
  const count = CounterContext.useSelector((snapshot) => snapshot.context.count) // [!code focus]
  return <span>{count}</span>
}
```
...
- **`@owanturist/signal`** - Natural isolation. Create signals inside a component and they are scoped to that instance. Share via props or context as needed. No global state exists unless you explicitly define it at module scope.
...
function Counter() {
  const monitor = useMonitor() // [!code focus:2]
  const value = count.read(monitor)

  return (
    <div>
      <span>{value}</span>
      <button onClick={() => count.write(5)}>Set 5</button> // [!code focus:2]
...
const counterSlice = createSlice({ // [!code focus:8]
  name: "counter",
  initialState: { count: 0 },
  reducers: {
    increment: (state) => { state.count += 1 },
...
type RootState = ReturnType<typeof store.getState> // [!code focus]

function Counter() {
  const value = useSelector((state: RootState) => state.counter.count) // [!code focus]
  const dispatch = useDispatch() // [!code focus]

  return (
    <div>
      <span>{value}</span>
      <button onClick={() => dispatch(counterSlice.actions.set(5))}>Set 5</button> // [!code focus:2]
...
function App() {
  return (
    <Provider store={store}> // [!code focus:3]
      <Counter />
    </Provider>
  )
}
...
const useCounter = create<{ // [!code focus:9]
  count: number
  set: (n: number) => void
  increment: () => void
}>((set) => ({
...
function Counter() {
  const value = useCounter((state) => state.count) // [!code focus:3]
  ...
  return (
    <div>
      <span>{value}</span>
      <button onClick={() => set(5)}>Set 5</button> // [!code focus:2]
...
function Counter() {
  const value = useAtomValue(countAtom) // [!code focus]
  const setCount = useSetAtom(countAtom) // [!code focus]

  return (
    <div>
      <span>{value}</span>
      <button onClick={() => setCount(5)}>Set 5</button> // [!code focus:2]
...
const counterMachine = setup({ // [!code focus:18]
  types: {
...
function Counter() {
  const actorRef = useActorRef(counterMachine) // [!code focus:2]
  const value = useSelector(actorRef, (snapshot) => snapshot.context.count)

  return (
    <div>
      <span>{value}</span>
      <button onClick={() => actorRef.send({ type: "set", value: 5 })}>Set 5</button> // [!code focus:2]
...
const firstName = Signal("John") // [!code focus:2]
const lastName = Signal("Doe")

function FullName() {
  // Subscribes to both firstName and lastName
  const full = useComputed((monitor) => { // [!code focus:3]
...
function FirstNameOnly() {
  ...
  const first = useComputed(firstName) // [!code focus]
...
function FullName() {
  ...
  const full = useSelector( // [!code focus:3]
...
function FirstNameOnly() {
  ...
  const first = useSelector((state: RootState) => state.user.firstName) // [!code focus]
  return <span>{first}</span>
}
```
...
function FullName() {
  ...
  const full = useUserStore( // [!code focus:3]
...
function FirstNameOnly() {
  const first = useUserStore((state) => state.firstName) // [!code focus]
  return <span>{first}</span>
}
```
...
const firstNameAtom = atom("John") // [!code focus:5]
...
function FullName() {
  ...
  const full = useAtomValue(fullNameAtom) // [!code focus]
  return <span>{full}</span>
}

function FirstNameOnly() {
  // Subscribes to firstNameAtom only
  const first = useAtomValue(firstNameAtom) // [!code focus]
  return <span>{first}</span>
}
```
...
function FullName({ actorRef }: { actorRef: ActorRefFrom<typeof userMachine> }) {
  const full = useSelector( // [!code focus:4]
    actorRef,
...
function FirstNameOnly({ actorRef }: { actorRef: ActorRefFrom<typeof userMachine> }) {
  const first = useSelector(actorRef, (snapshot) => snapshot.context.firstName) // [!code focus]
  return <span>{first}</span>
}
```
...
const count = Signal(0) // [!code focus:3]
...
const selectCount = (state: RootState) => state.counter.count // [!code focus:3]
...
function Component() {
  const doubled = useCounter((state) => state.count * 2) // [!code focus:2]
...
const countAtom = atom(0) // [!code focus:3]
...
const selectDoubled = (state) => state.context.count * 2 // [!code focus:2]
...
const count = Signal(0)               // WritableSignal<number> // [!code focus:3]
...
const userSlice = createSlice({ // [!code focus:9]
  name: "user",
...
const store = configureStore({ reducer: { user: userSlice.reducer } }) // [!code focus]

// Manual type definitions required
type RootState = ReturnType<typeof store.getState> // [!code focus:2]
type AppDispatch = typeof store.dispatch

// Pre-typed hooks to avoid repeating RootState everywhere
const useAppSelector = useSelector.withTypes<RootState>() // [!code focus:2]
...
const useStore = create<{ // [!code focus:9]
  name: string
  age: number
...
const nameAtom = atom("Alice")              // PrimitiveAtom<string> // [!code focus:5]
...
const userMachine = setup({ // [!code focus:14]
  types: {
...
**Choose Zustand when** you want the simplest possible external store. Zustand's minimal API and tiny bundle make it a good default for applications that do not need fine-grained reactivity or complex state machines.
