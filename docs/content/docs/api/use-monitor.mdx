...
The `useMonitor{:dart}` hook is the simpler primitive - it returns a Monitor and leaves reading and rendering logic to you. The `useComputed{:dart}` hook builds on top of it, adding memoization, dependency arrays, and custom equality checks.
