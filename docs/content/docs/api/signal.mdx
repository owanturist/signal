---
title: Signal
description: API reference for the Signal factory and Signal types
---

The `Signal{:dart}` function creates reactive signals that hold values and notify consumers when changed.
...
- `@returns ReadonlySignal<TDerivedValue>{:dart}` A read-only signal that re-computes when dependencies change.

</section>

```ts tab="Function Getter"
import { Signal, effect } from "@owanturist/signal"
...
- `@param equals?: null | Equal<T>{:dart}` Custom equality function to determine if a value has changed. Pass `null{:ts}` to use the default `Object.is{:ts}`. When the function returns `true{:ts}`, consumers are not notified.
...
Reads the signal's value and registers the monitor as a consumer.

<section className="typedef">

- `@param monitor: Monitor{:dart}` The monitor to register as a consumer.
---
...
Updates the signal's value and notifies all consumers.
